...
def epsilon_greedy_policy(Q, epsilon, state, num_actions):
    """epsilon-greedy-policy(Q, epsilon, s_t) -> (a, P(a)), per the algorithm spec."""
    u = random.random()
    if u < epsilon:
        a = random.randrange(num_actions)
        p_a = epsilon / num_actions
    else:
        action_values = [Q[(state, a)] for a in range(num_actions)]
        a = int(np.argmax(action_values))
        p_a = 1 - epsilon + epsilon / num_actions
    return a, p_a


def sarsa_control(env, num_steps, gamma, alpha, epsilon):
    Q = defaultdict(float)
    state, _ = env.reset()
    action, _ = epsilon_greedy_policy(Q, epsilon, state, num_actions)
    i = 0
    while i < num_steps:
        next_state, reward, terminated, truncated, _ = env.step(action)
        i += 1
        next_action, _ = epsilon_greedy_policy(Q, epsilon, next_state, num_actions)
...
# Run SARSA control with epsilon-greedy exploration
Q = sarsa_control(env, num_steps, gamma, alpha, epsilon)
