...
# Special states
trap = (2, 2)    # (row=1, col=3) in 0-indexed Python -> position (2,4) in human terms
goal = (2, 1)    # (row=0, col=3) -> position (1,4)
...
def bellman_update(state, V, gamma):
    ...
    if state == goal or state == trap:
        return V[state]  # terminal states keep their reward
    values = []
...
print(f"Final state-value function after N iterations:")
print(final_values)