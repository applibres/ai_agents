...
def mc_first_visit_action_value_incremental(env, num_episodes, gamma=0.9):
    Q = defaultdict(float)
    N = defaultdict(int)
    ...
    for ep in range(num_episodes):
        ...
        for t in reversed(range(len(episode))):
            ...
            if pair not in visited_pairs:
                N[pair] += 1
                Q[pair] += (1 / N[pair]) * (G - Q[pair])
                visited_pairs.add(pair)
    return Q

# Run MC Evaluation
Q = mc_first_visit_action_value_incremental(env, num_episodes, gamma)
